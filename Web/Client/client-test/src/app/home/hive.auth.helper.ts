  ...
  private generateAuthPresentationJWT(appDid: string, authChallengeJwttoken: string): Promise<string> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        let claims: Claims = (await new JWTParserBuilder().setAllowedClockSkewSeconds(300).build().parse(authChallengeJwttoken)).getBody();
        if (claims == null)
        ...
        if (presentation) {
          ...
          try {
            ...
            try {
              ...
              let jwtToken = await didDocument.jwtBuilder().addHeader(JWTHeader.TYPE, JWTHeader.JWT_TYPE)
